...
def test_upload_media_success(client):
    ...
    dummy_file_path = "test_image.jpg"
    with open(dummy_file_path, "wb") as f:
        f.write(os.urandom(1024))  # Create a dummy file

    with open(dummy_file_path, "rb") as file:
        data = {
            "media": (file, "test_image.jpg")
        }
        response = client.post("/upload_media", data=data, content_type="multipart/form-data")
        assert response.status_code == 200
        assert response.json["success"] is True
        assert "uploads/" in response.json["url"]
...
def test_upload_media_no_file(client):
    ...
    response = client.post("/upload_media", data={}, content_type="multipart/form-data")
    assert response.status_code == 200
    assert response.json["success"] is False
